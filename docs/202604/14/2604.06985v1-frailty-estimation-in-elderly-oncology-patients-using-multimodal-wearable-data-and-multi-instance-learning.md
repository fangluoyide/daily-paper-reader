...
tags: ["query:cog-eld"]
score: 6.0
evidence: 老年患者的衰弱与功能下降评估
tldr: 本研究针对老年癌症患者衰弱评估频率不足的问题，提出一种基于多模态可穿戴数据的监测框架。通过融合智能手表的活动与睡眠数据以及胸带的心率变异性（HRV）特征，利用注意力机制的多示例学习（MIL）模型处理现实中数据缺失和不规则采样问题，旨在预测患者在随访期间的功能状态变化（如手握力和疲劳程度）。该方法为老年肿瘤患者的远程、持续衰弱评估提供了有效的技术手段。
source: arxiv
...
method: 采用基于注意力机制的多示例学习框架，通过模态特定的编码器融合不规则且存在缺失的纵向可穿戴设备数据。
result: 模型在预测手握力和疲劳量表变化方面表现良好，其中手握力预测在6个月随访时的平衡准确率达到0.70。
conclusion: 智能手表监测的活动与睡眠数据对衰弱变化最具预测性，而HRV特征在多模态融合中起到了关键的补充作用。
---
