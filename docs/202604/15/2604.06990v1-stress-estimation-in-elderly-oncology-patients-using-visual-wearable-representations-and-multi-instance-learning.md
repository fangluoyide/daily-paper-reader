...
## 摘要
心理压力在心脏肿瘤学中具有临床相关性，但通常仅通过患者报告结局指标 (PROMs) 进行评估，且很少被整合到持续的心脏毒性监测中。本研究利用来自智能手表（身体活动和睡眠）和胸戴式心电图 (ECG) 传感器的多模态可穿戴数据，评估了一个老年多中心乳腺癌队列 (CARDIOCARE) 的感知压力。可穿戴数据流被转化为异构的视觉表示，从而产生了一个弱监督环境，其中单个感知压力量表 (PSS) 评分对应于许多未标记的时间窗。一个轻量级预训练混合专家骨干网络 (Tiny-BioMoE) 将每个表示嵌入到 192 维向量中，这些向量通过基于注意力的多实例学习 (MIL) 进行聚合，以预测第 3 个月 (M3) 和第 6 个月 (M6) 的 PSS。在留一受试者 (LOSO) 评估下，预测结果与问卷评分显示出中度一致性（M3：R^2=0.24，Pearson r=0.42，Spearman rho=0.48；M6：R^2=0.28，Pearson r=0.49，Spearman rho=0.52），M3 的全局 RMSE/MAE 分别为 6.62/6.07，M6 为 6.13/5.54。
